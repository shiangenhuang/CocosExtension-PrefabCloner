...
function getAllFiles(dir: string, ext: string): string[] {
    ...
    try {
        ...
        for (const entry of entries) {
            ...
            if (['node_modules', '.git', 'library', 'temp'].includes(entry.name)) continue;
...
function buildMetaCache(searchDir: string): Record<string, string> {
...
function findMetaByUUID(searchDir: string, uuid: string): string | null {
...
export function clonePrefab(
    ...
    const externalUUIDs = new Set<string>();
    ...
    const fileCopyMap: Record<string, FileCopyInfo> = {};
    ...
    for (const [source, info] of Object.entries(fileCopyMap)) {
        ...
        let newMetaContent = metaContent.replace(
            new RegExp(oldMainUUID.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'g'),
            info.newUUID,
        );
    ...
    for (const [oldUUID, newUUID] of sortedEntries) {
        ...
        if (before !== newPrefabContent) replacements++;
    }
    ...
    const remainingCrossRefs: { uuid: string; source: string }[] = [];
